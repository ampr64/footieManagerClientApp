...
export class LeagueService {
    ...
    getByCountry(countryId: number): Observable<ILeague[]> {
        let url_ = this._baseUrl + "api/Leagues/countries/{countryId}";
    ...
    private processGetByCountry(response: HttpResponseBase): Observable<ILeague[]> {
        const status = response.status;
        const responseBlob =
            response instanceof HttpResponse ? response.body :
                (<any>response).error instanceof Blob ? (<any>response).error : undefined;
